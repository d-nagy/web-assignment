...
const getWorkout = (slug, done) => {
  const workout = workouts.find((wk) => wk.slug === slug);
  if (workout) {
...
const getWorkoutOfTheDay = (done) => {
  const workout = workouts.find((wk) => wk.wotd === true);
  if (workout) {
    return done(null, 200, workout);
  }
  const error = new Error('No workout of the day set');
  error.name = 'noWotd';
...
const getMostRecentWorkout = (username, done) => {
  const completed = CompletedWorkout.getCompletedWorkoutsByUsername(username);

  if (completed.length > 0) {
    const recent = Math.max(...completed.map((c) => c.last_completed));
    const mostRecentSlug = completed.find((c) => c.last_completed === recent).slug;

    getWorkout(mostRecentSlug, (err, status, result) => {
      if (!err) {
        const workoutWithTime = Object.keys(result).reduce((object, key) => {
...
const getMostCompletedWorkout = (username, done) => {
  ...
  if (completed.length > 0) {
    const count = Math.max(...completed.map((c) => c.count));
    const mostCompletedSlug = completed.find((c) => c.count === count).slug;

    getWorkout(mostCompletedSlug, (err, status, result) => {
      if (!err) {
        const workoutWithCount = Object.keys(result).reduce((object, key) => {
          object[key] = result[key];
          return object;
        }, {});
