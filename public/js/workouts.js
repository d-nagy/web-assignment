...
function deleteWorkout(slug) {
  $.ajax({
    ...
    $('#removeBlockModal .modal-body')
        .html('Cannot delete workout: ' + jqXHR.responseText);
...
function createWorkoutCard(i, wk, exercises) {
  ...
  $.each(wk.routine, function(i, item) {
    switch (item.type) {
      ...
      case 'exercise-single':
        ...
        const ex = exercises.find(function(element) {
        ...
        $.each(item.exercises, function(i, ex) {
          switch (ex.type) {
            ...
            case 'exercise-single':
              ...
              const e = exercises.find(function(element) {
              ...
              $subli.find('h6')
                  .html(`${e.name} &times ${ex.quantity} ${ex.units}`);
  ...
  const exDataTarget = 'exerciseCollapse-' + i;
  const wkDataTarget = 'workoutCollapse-' + i;
  ...
  if ($wkCard.find('.card-footer').length) {
    const fav = $wkCard.find('.card-footer .fav-button');
    ...
    fav[0].addEventListener('click', function(e) {
      ...
      if ($(this).hasClass('favourite')) {
        $(`.fav-button[data-id=${wk.slug}]`)
            .removeClass('favourite').addClass('unfavourite');
        favourite = -1;
      } else {
        $(`.fav-button[data-id=${wk.slug}]`)
            .removeClass('unfavourite').addClass('favourite');
      }
      setFavourite($(this).attr('data-id'), favourite);
    });

    const complete = $wkCard.find('.card-footer .wkCompleteButton');
    complete.attr('data-id', wk.slug);

    if (wk.can_complete) {
      complete[0].addEventListener('click', function(e) {
        $(`.wkCompleteButton[data-id=${wk.slug}]`)
            .removeClass('btn-secondary')
            .addClass('active disabled btn-success');
        $(`.wkCompleteButton[data-id=${wk.slug}]`)
            .html('Times completed: ' + wk.complete_count);
        completeWorkout($(this).attr('data-id'));
      });
    } else {
      complete.removeClass('btn-secondary')
          .addClass('active disabled btn-success');
...
function populateWorkoutResults(data, textStatus, jqXHR) {
  ...
  $.each(workouts, function(i, wk) {
    ...
    if (wk.wotd === true) {
      ...
      if (wk.can_complete) {
        ...
        if ($wkCard.find('.card-footer').length) {
          ...
          $wotdCard.find('.card-footer .fav-button')[0]
          ...
          const complete = $wotdCard.find('.card-footer .wkCompleteButton');
          complete.attr('data-id', wk.slug);
          if (wk.can_complete) {
            complete[0].addEventListener('click', function(e) {
              $(`.wkCompleteButton[data-id=${wk.slug}]`)
        ...
        const exDataTarget = 'exerciseCollapse-wotd';
        const wkDataTarget = 'workoutCollapse-wotd';

        $wotdCard.find('.exCollapseButton')
            .attr('data-target', '#' + exDataTarget);
        $wotdCard.find('.exerciseDisplay').attr('id', exDataTarget);

        $wotdCard.find('.wkCollapseButton')
            .attr('data-target', '#' + wkDataTarget);
    ...
    if (wk.complete_count < 1) {
      ...
      if ($wkCard.find('.card-footer').length) {
        $uncompleteCard.find('.card-footer .fav-button')
            .attr('data-id', wk.slug);
        $uncompleteCard.find('.card-footer .fav-button')[0]
            .addEventListener('click', function(e) {
              let favourite = 1;
              if ($(this).hasClass('favourite')) {
                $(`.fav-button[data-id=${wk.slug}]`)
        ...
        const complete = $uncompleteCard.find('.card-footer .wkCompleteButton');
        complete.attr('data-id', wk.slug);
      ...
      const exDataTarget = 'exerciseCollapse-uncomplete-' + i;
      const wkDataTarget = 'workoutCollapse-uncomplete-' + i;

      $uncompleteCard.find('.exCollapseButton')
          .attr('data-target', '#' + exDataTarget);
      $uncompleteCard.find('.exerciseDisplay').attr('id', exDataTarget);

      $uncompleteCard.find('.wkCollapseButton')
          .attr('data-target', '#' + wkDataTarget);
        ...
        if (checkValidWorkout() !== false && formValid !== false) {
          ...
          $.ajax({
            ...
            $(form).children('.alert-success')
                .html('Workout added!').show().delay(5000).fadeOut(200);
...
$('#setWotdModal').on('show.bs.modal', function(event) {
  ...
  const slug = button.data('id');
  const modal = $(this);
  modal.find('#setWotdSlug').val(slug);
});

$('#setWotdConfirmButton').click(function(event) {
  const modal = $('#setWotdModal');
  const slug = modal.find('#setWotdSlug').val();
